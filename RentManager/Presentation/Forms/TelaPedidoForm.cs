...
namespace RentManager
{
    public partial class TelaPedidoForm : Form
    {
        ...
        public TelaPedidoForm(RentManagerDataBase db)
        {
            InitializeComponent();
            _db = db;
        ...
        public TelaPedidoForm(RentManagerDataBase db, int pedidoId) : this(db)
        {
            _pedidoId = pedidoId;
        ...
        private void CarregarPedido(int pedidoId)
        {
            ...
            try
            {
                ...
                ?? (pedido.Cliente is ClientePJ pjCli ? pjCli.RazaoSocial : "");
        ...
        private static string FormatarTelefone(string telefone)
        {
            ...
            {
                11 => $"({numeros[..2]}){numeros[2..7]}-{numeros[7..]}",   // celular: (99)99999-9999
                10 => $"({numeros[..2]}){numeros[2..6]}-{numeros[6..]}",   // fixo:    (99)9999-9999
                9 => $"{numeros[..5]}-{numeros[5..]}",                    // sem DDD celular
                8 => $"{numeros[..4]}-{numeros[4..]}",                    // sem DDD fixo
                _ => telefone                                              // não reconhecido, deixa como está
            };
        }
        #endregion
        ...
        private void AtualizarTotais()
        {
            ...
            decimal.TryParse(designTextBoxDesconto.Text, out desconto);
            decimal.TryParse(
    designTextBoxTotalValorQuebra.Text,
    ...
    out valorQuebra);
        ...
        private DadosPedidoImpressao ObterDadosParaImpressao()
        {
            var dados = new DadosPedidoImpressao
            {
                ...
                ContatoNumero = FormatarTelefone(designTextBoxNumeroContato.Text),
        ...
        private void VisualizarPDF()
        {
            ...
            {
                // Validações básicas
                bool isOrcamento = string.IsNullOrWhiteSpace(designTextBoxNomeCliente.Text);

                if (isOrcamento)
                {
                    var confirmar = MessageBox.Show(
                        "Nenhum cliente selecionado.\nDeseja gerar um ORÇAMENTO?",
                        "Orçamento",
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question);
        ...
        private void designButtonFinalizarPedido_Click(object sender, EventArgs e)
        {
            ...
            if (_pedidoId.HasValue)
            {
                ...
                foreach (DataGridViewRow row in dataGridViewProdutosLocacao.Rows)
                {
                    ...
                    {
                        db.ProdutosPedidos.Add(new ProdutoPedido
                        {
                            PedidoId = pedido.ID,
                            ProdutoId = produtoId,
                ...
                {
                    var saldoRemovido = db.SaldosPorData.FirstOrDefault(
                        s => s.ProdutoId == itemRemovido.ProdutoId && s.Data.Date == dataPedido);
                    if (saldoRemovido != null)
                        saldoRemovido.QuantidadeReservada -= itemRemovido.Quantidade;

                    db.ProdutosPedidos.Remove(itemRemovido);
                }
            ...
            else
            {
                ...
                foreach (DataGridViewRow row in dataGridViewProdutosLocacao.Rows)
                {
                    ...
                    // CORREÇÃO: validação segura do ProdutoID
                    ...
                    if (!int.TryParse(row.Cells["Quantidade"].Value?.ToString(), out int quantidade) || quantidade <= 0)
                        continue;
    ...
    .Select(r => new ProdutoQuebra
    {
        ...
        private async void designButtonEmitirNota_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string descricaoPadrao =
                    $"Valor referente a locação de materiais conforme pedido N° {_pedidoId}"; 
        ...
        private void designButtonGerarContrato_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(designTextBoxNomeCliente.Text))
            {
                MessageBox.Show("Selecione um cliente antes de gerar o contrato.",
                    "Cliente obrigatório", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            var opcao = MessageBox.Show(
                "Deseja imprimir o contrato diretamente?\n\nSim = Imprimir\nNão = Visualizar PDF",
                "Gerar Contrato",
                MessageBoxButtons.YesNoCancel,
                MessageBoxIcon.Question);
            ...
            if (opcao == DialogResult.Yes)
                ContratoLocacaoService.ImprimirDireto(dados);
            else
                ContratoLocacaoService.VisualizarPDF(dados);
